...
%\include{markov-intro}
%\include{markov-ruin}
%\include{markov-walk}
%\include{markov-ehren}
%\include{markov-two-state}
%\include{markov-maze}
%\include{markov-bib}
...
This problem and its solution is presented in \cite[Sections~5.3, 7.2]{privault}.
...
Privault computes the expected number of returns to state $0$ starting in any other state and the average number of steps in the simulation is very close to to the expected number of returns:
\begin{verbatim}
...
\end{verbatim}
Privault also computes the expected number of returns to state $i$ from state $i$ and the averages (the main diagonal of the above matrix) are very close:
\begin{verbatim}
